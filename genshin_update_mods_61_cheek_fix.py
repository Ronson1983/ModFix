		...
		if 'Texture' in my_block['block_name'] and 'Face' in my_block['block_name']: #and 'Head' in my_block['block_name']:
			...
			for idx, edit_line in enumerate(my_block['lines']):
				if edit_line.startswith('ps-t0'):
					my_block['lines'][idx] = edit_line.replace('ps-t0', 'this')
					has_ps_t0 = True
					
				if edit_line.startswith('run'):
					block_name = edit_line.replace('run =', '').strip()
					if block_name in commandLists:
						follow_blocks.append(block_name)
